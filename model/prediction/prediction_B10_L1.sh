...
CUDA_VISIBLE_DEVICES=`/opt/Tools/bin/first-free-gpu|head -1` python -m bin.infer \
  --tasks "
    ...
    - class: DumpBeams
      params:
        file: ${PRED_DIR}/beams_10_LP1.npz" \
  --model_dir $MODEL_DIR \
  --model_params "
  ...
  --input_pipeline "
    ...
    params:
      ...
      source_files:
        - $TEST_SOURCES" \
  > ${PRED_DIR}/predictions_beam_10_LP1.txt
./bin/tools/multi-bleu.perl ${DEV_TARGETS_REF} < ${PRED_DIR}/predictions_beam_10_LP1.txt > ${PRED_DIR}/results_beam_10_LP1.txt
cd $CODE_DIR